...
## Idiomatic Go Improvements for `cmd.go`
**Provenance**
- Generated by AI assistant: opencode
...
Go standard practice is `camelCase` for internal functions and `PascalCase` for exported ones.
- **Change:** `get_inputFiles` → `getInputFiles`
- **Change:** `process_single_file` → `processSingleFile`

### 2. Resource Management (Leaking File Descriptors)
In `get_inputFiles`, files are opened but never closed. This will eventually hit system limits if many files are processed.
- **Recommendation:** Open the file inside the processing loop (in `ipcmd`) and use `defer file.Close()`.

### 3. Slice Initialization
- **Change:** `var iFiles = []inputFile{}` → `var iFiles []inputFile`
- **Reason:** Declaring a `nil` slice is the idiomatic way to start a slice that will be grown with `append`. It avoids unnecessary allocation if no items are added.

### 4. Use `io.ReadCloser` for Abstraction
The `inputFile` struct currently uses a `bool` to track `IsStdin`.
- **Recommendation:** Store an `io.ReadCloser` (like `*os.File` or `os.Stdin`) directly in the struct. This allows the processing logic to treat all inputs uniformly without checking flags.

### 5. Efficient Output Writing
In `displayOutput`, `fmt.Print(string(b))` creates a copy of the JSON byte slice.
- **Recommendation:** Use `os.Stdout.Write(b)` to write the bytes directly to standard output.

### 6. Documentation and Style
- Use `//` for function documentation instead of `/* ... */` to follow `go doc` standards.
- Remove end-of-function comments like `// func ipcmd`, as they are not standard in Go.
- Ensure all functions return errors instead of using `panic`.
